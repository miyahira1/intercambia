    ...
    <div className="flex h-[calc(100vh-88px)]">
      ...
      <div className="w-full md:w-72 border-r border-gray-100 bg-white flex flex-col shrink-0">
        <div className="p-4 border-b border-gray-100">
          <h1 className="font-bold text-gray-900">{m.title}</h1>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3">
              ...
              <div className="max-w-[70%]">
                <div
                  className={`rounded-2xl px-4 py-2.5 text-sm ${
                    msg.from === 'self' ? 'bg-[#1E40AF] text-white' : 'bg-gray-100 text-gray-800'
                  }`}
                >
                  <span className="cursor-pointer hover:underline hover:decoration-dotted">{msg.text}</span>
                </div>
                <div className={`text-xs text-gray-400 mt-1 ${msg.from === 'self' ? 'text-right' : 'text-left'}`}>
                  {msg.time} {msg.from === 'self' && '✓✓'}
                </div>
              </div>
            </div>
          ))}
          ...
          <div className="flex justify-center">
            <div className="bg-gray-800 text-white text-xs rounded-xl px-4 py-3 max-w-xs text-center shadow-lg">
              <div className="font-medium mb-1">{m.tooltip('works')}</div>
