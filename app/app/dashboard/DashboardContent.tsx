...
export default function DashboardContent() {
  ...
  const stats = [
    { label: d.xpTotal, value: '340' },
    { label: d.sessions, value: '7' },
    { label: d.streak, value: '3 sem.' },
  ]

  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">{d.greeting}</h1>
      ...
      <section className="mb-10">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">{d.recommendedMatches}</h2>
          ...
          {matches.map((m) => (
            <div key={m.name} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="flex gap-2">
                <button className="flex-1 text-xs bg-[#1E40AF] text-white py-2 rounded-lg hover:bg-[#1e3a8a] transition-colors">{d.connect}</button>
                <button className="flex-1 text-xs border border-gray-200 text-gray-600 py-2 rounded-lg hover:bg-gray-50 transition-colors">{d.viewProfile}</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <section>
        <h2 className="text-lg font-semibold text-gray-900 mb-4">{d.upcomingSessions}</h2>
